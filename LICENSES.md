...
| Source | Material used | Status |
| --- | --- | --- |
| [`local/universities-ru-overlay`](https://github.com/akent4000/ru-routing/blob/main/config/overlays/universities-ru.txt) | curated Russian university domain additions | Repository-maintained routing policy data; not third-party licensed material. |
...
| [fatyzzz/max-list](https://github.com/fatyzzz/max-list) | `ip-check-formatted.txt` IP-check domains and `vpndetect.txt` connectivity-check domains | **No repository license declaration found.** The GitHub API reports no license, the repository has no license file, and the README states no redistribution terms. The generated release lists are published without a license declaration. | fatyzzz/max-list contributors |
