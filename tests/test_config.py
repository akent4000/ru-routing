...
def test_loaded_policies_are_immutable_and_preserve_freshness_and_tiers():
    ...
    assert policy.source_categories[
        "hydraponique/roscomvpn-geoip:direct"
    ].datasets == frozenset({"lite", "server"})
    assert policy.source_categories[
        "hydraponique/roscomvpn-geoip:whitelist"
    ].datasets == frozenset({"lite", "server"})
    ...
    assert source_removal.expected_current_policy_fingerprint == (
        "542df05e6553803731fd959eb0caba03bd5e687d40ec1b13189cd1611232ae46"
    )
    ...
    geoip_scope = category_scope_by_previous_policy[
    ...
    assert geoip_scope.reset_size is True
